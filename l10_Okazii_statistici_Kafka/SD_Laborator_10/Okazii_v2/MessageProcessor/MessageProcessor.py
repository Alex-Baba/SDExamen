...
class MessageProcessor:
    def __init__(self, bids_topic, notify_message_processor_topic, processed_bids_topic, monitor_topic):
        super().__init__()
    ...
    def sendToMonitor(self,msg):
        msg_headers = [
            ("timestamp", bytes(str(datetime.now()), encoding="utf-8")),
            ("identity", bytes("MessageProcessor", encoding="utf-8"))
        ]
    ...
    def finish_processing(self, sorted_bids):
        ...
        for bid in sorted_bids:
            ...
            self.sendToMonitor(bytes(f'{identity}{bid.value}~{bid_amount}', encoding='utf-8'))
