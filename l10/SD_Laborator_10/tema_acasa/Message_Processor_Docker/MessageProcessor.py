...
class MessageProcessor:
    ...
    def get_and_process_messages(self):
        ...
        if str(auction_end_message.value, encoding="utf-8") == "incheiat":
            ...
            self.auction_id = auction_end_message.headers[0][1]
            print(f"Licitatia {self.auction_id} incheiata. Procesez mesajele cu oferte...")
    ...
    def finish_processing(self, sorted_bids):
        ...
        if len(sorted_bids) == 0:
            self.processed_bids_producer.send(topic=self.processed_bids_topic, value=bytes("empty", encoding="utf-8"))
