...
// configurarea Spark
    val sparkConf = SparkConf().setMaster("local").setAppName("SparkExample")
...
// 1) spargerea fiecarui string din lista intr-o lista desubstring-uri si reunirea intr-o singura lista
// 2) filtrarea cu regex pentru a pastra doar numerele
// 3) conversia string-urilor filtrate la int prin functiademapare
...
// setul de date nu este inca incarcat in memorie (si nu seactioneaza inca asupra lui)
    val lines = sparkContext.textFile("src/main/resources/data.txt")// pentru utilizarea unui RDD de mai multe ori, trebuie apelatametoda persist:
    lines.persist(StorageLevel.MEMORY_ONLY())
// functia de mapare reprezinta o transformare a setului dedateinitial (nu este calculat imediat)
// abia cand se ajunge la functia de reducere (care esteoactiune)Spark imparte operatiile in task-uri
// pentru a fi rulate pe masini separate (fiecare masina executando parte din map si reduce)
// exemplu cu functii lambda:
    val totalLength0 = lines.map { s->s.length }.reduce { a: Int,b:Int -> a + b }
// trimiterea unor functii catre Spark
    val totalLength1= lines.map(object : Function<String?, Int?>{override fun call(p0: String?): Int? {
        return p0?.length ?: 0
    }
    ...
    // sau daca scrierea functiilor inline sau a celor lambdaestegreoaie, se pot utiliza clase
...
// trimiterea unui set de date ca input catre fiecare nodintr-omaniera eficienta:
