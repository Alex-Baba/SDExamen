...
class LibraryApp(QWidget):
    ...
    def search(self):
        ...
        if not search_string:
            ...
            if self.json_rb.isChecked():
                url = '/find?format=json&'
            elif self.html_rb.isChecked():
                url = '/find?format=html&'
            else:
                url = '/find?format=raw&'
            ...
            url = url[:-1]
            print(url)
        full_url = "http://localhost:7000" + url
        try:
            self.rabbitmq.send_message(url)
            self.rabbitmq.receive_message()
            print(self.result_msg)
            if 'HIT' in self.result_msg:
                self.result.setText(self.result_msg[self.result_msg.find('~') + 1:])
            elif 'MISS' in self.result_msg:
                response = requests.get(full_url)
                self.result.setText(response.content.decode('utf-8'))
            else:
                raise Exception("Mesaj prost de pe Rabbit")
