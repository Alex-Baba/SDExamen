                ...
                while (true) {
                    ...
                    when (messageType) {
                        "intrebare" -> {
                            ...
                            monitorSocket.let {
                               var monitorStream = monitorSocket?.getOutputStream()
                                monitorStream?.write("startConvo\n".toByteArray())
                                monitorStream?.write("$receivedMessage\n".toByteArray())
                            }
                        }
                        "raspuns" -> {
                            ...
                            monitorSocket.let {
                                var monitorStream = monitorSocket?.getOutputStream()
                                monitorStream?.write("$receivedMessage\n".toByteArray())
                                monitorStream?.write("endConvo\n".toByteArray())
                            }
                        }
                        "command" -> {
                            //Monitorul isi sa locul de oanoare

                            synchronized(subscribers) {
                                subscribers.remove(clientConnection.port)
                                monitorSocket = clientConnection
                            }
                            println("Monitor is in place.")
                        }
                    }
                }
            }
