...
class MessageManagerMicroservice {
    ...
    private lateinit var registerSocket: Socket

    companion object Constants {
        const val MESSAGE_MANAGER_PORT = 1500
        val MESSAGE_HOST = System.getenv("MESSAGE_MANAGER_HOST") ?: "localhost"
        val REGISTER_HOST = System.getenv("REGISTER_HOST") ?: "localhost"
    ...
    private fun subToRegister() {
        registerSocket = Socket(REGISTER_HOST, REGISTER_PORT)
        registerSocket.getOutputStream().write("sub~{\"name\":\"message_manager\",\"host\":\"$MESSAGE_HOST\",\"port\":\"$MESSAGE_MANAGER_PORT\"}\n".toByteArray())
    }

    private fun unsubToRegistrar() {
        registerSocket.getOutputStream().write("unsub~\n".toByteArray())
    }
