...
class MessageManagerMicroservice {
    ...
    private fun respondTo(destination: Int, receivedMessage: String) {
        println("Trimitel la destinatia: " + destination)
        subscribers[destination]?.getOutputStream()?.write((receivedMessage + "\n").toByteArray())
    }
                ...
                while (true) {
                    ...
                    when (messageType) {
                        "intrebare" -> {
                            ...
                            broadcastMessage("intrebare ${clientConnection.port} ${sourceTeacher} $messageBody")
                        }
                        "raspuns" -> {
                            // tipul mesajului de tip raspuns este de forma:
                            // raspuns <CONTINUT_RASPUNS>
                            respondTo(messageDestination.toInt(), receivedMessage)
                        }
                    }
                }
            }
