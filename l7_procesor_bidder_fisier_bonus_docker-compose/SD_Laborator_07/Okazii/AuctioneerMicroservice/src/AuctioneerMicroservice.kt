...
class AuctioneerMicroservice {
    ...
    private var monitorSocket: Socket? = null
    ...
    companion object Constants {
        ...
        const val MONITOR_HOST = "localhost"
        const val MONITOR_PORT = 10010
    }

    init {
        auctioneerSocket = ServerSocket(AUCTIONEER_PORT)
        auctioneerSocket.setSoTimeout(AUCTION_DURATION.toInt())
        monitorSocket = Socket(MONITOR_HOST, MONITOR_PORT)
    ...
    private fun sendToMonitor(msg: Message){
        val newMsg = Message.create("auctioneer", "$msg")
        monitorSocket?.getOutputStream()?.write(newMsg.serialize())
    }
        ...
        try {
            ...
            bidderConnections.forEach {
                if (it.remoteSocketAddress.toString() == result.sender) {
                    ...
                    sendToMonitor(winningMessage)
                } else {
                    it.getOutputStream().write(losingMessage.serialize())
                    sendToMonitor(losingMessage)
                }
                it.close()
            }
        } catch (e: Exception) {
